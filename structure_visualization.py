...
# ==========================================
# Updated based on lstm_structure.txt
LSTM_CONFIG = {
    "LSTM_UNITS": 56,
    "DROPOUT_RATE": 0.45,
    "EMBEDDING_DIM": 24,
    ...
    "DENSE_1": 16,
    "MERGE_DENSE": 64,
    # [UPDATED] Steps Head now matches text: Dense(64) -> Drop -> Dense(32)
    "STEPS_HEAD_HIDDEN_1": 64,
    "STEPS_HEAD_DROP_1": 0.45, # Assumed same rate
    "STEPS_HEAD_HIDDEN_2": 32,
...
# ==========================================
# 2. Transformer Model Configuration
# ==========================================
# Matches transformer_structure.txt
TRANS_CONFIG = {
    "PROJECT_DIM": 16,
    "NUM_HEADS": 6,
    "FF_DIM": 16,
...
def create_node(dot, name, label, shape='box', style='filled', color='white', fillcolor='white', **kwargs):
    """Helper to create nodes with consistent styling"""
...
def draw_lstm_model():
    ...
    # Layout settings: Left-to-Right, Orthogonal lines
    ...
    # --- Heads ---
    # Steps Head [UPDATED STRUCTURE]
    with dot.subgraph(name='cluster_steps') as c:
        c.attr(label='Steps Head', style='dotted', fontsize='18')
        # Added extra dense/drop layer to match txt file
        create_node(c, 'step_dense1', f"Dense {LSTM_CONFIG['STEPS_HEAD_HIDDEN_1']}", fillcolor='#B2DFDB')
        create_node(c, 'step_drop1', f"Drop {LSTM_CONFIG['STEPS_HEAD_DROP_1']}", shape='ellipse', fillcolor='#FFAB91')
        create_node(c, 'step_dense2', f"Dense {LSTM_CONFIG['STEPS_HEAD_HIDDEN_2']}", fillcolor='#B2DFDB')
        create_node(c, 'out_steps', "Out: Steps\n(Linear)", shape='doublecircle', fillcolor='#80CBC4')
        
        dot.edge('shared_drop', 'step_dense1')
        dot.edge('step_dense1', 'step_drop1')
        dot.edge('step_drop1', 'step_dense2')
        dot.edge('step_dense2', 'out_steps')
